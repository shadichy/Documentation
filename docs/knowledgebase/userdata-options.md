...
{{ read_csv('../assets/csv/userdata/userdata.csv') }}
...
- Support `Apply from Sdcard` when using [Recovery Mode](../configuration/recovery.md). Because `/data` is already bind-mounted on initrd, we don't have to do anything extra in Recovery Mode and can just open the directory using the option.
